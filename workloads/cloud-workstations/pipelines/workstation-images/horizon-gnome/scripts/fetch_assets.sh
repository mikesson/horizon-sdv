...
fetch_images() {
  ...
  for image in ${GUACAMOLE_IMAGES}; do
    # Must match the reference used by `docker run` in guacd.service /
    # guacamole.service (rendered from CONTAINER_REGISTRY), otherwise the
    # tarball's RepoTags won't match what's requested at boot and the image
    # gets re-pulled from the network (which block-traffic.service blocks).
    ...
    if [[ ! -s "/downloads/opt/images/${image}.tar" ]]; then
      echo "ERROR: failed to pull ${image_id} after ${RETRIES} attempts" >&2
      exit 1
    fi
  done
}
