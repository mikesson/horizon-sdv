...
"""REST helpers for ``cf_create_instance_template.sh`` (metadata bearer token only).
...
Scope
    **Compute Engine v1:** image read/delete (global ``images.get`` /
    ``images.delete``), instance template read (regional then global ``GET``),
...
Subcommands (stdout / exit codes)
    ...
    ``get-instance-template <project> <region> <template_name>``
        ``GET .../regions/{region}/instanceTemplates/{name}``; if HTTP **404**,
        ``GET .../global/instanceTemplates/{name}``. Exit **0** if either returns
        **200** (stage **2** preflight: GCP template exists even when the KCC CR
        is missing). Exit **1** if neither exists or on transport / non-404 HTTP
        error.
...
# Shell export name; must match cf_create_instance_template.sh / cf_environment.sh.
...
def _http(
    ...
    *,
    ...
    """Return (HTTP status, response body text). HTTPError becomes (code, body); OSError propagates."""
...
def _os_login_login_profile_url(token: str) -> str:
    ...
    ``users/me`` only matches end-user credentials. GKE workload identity returns a
    service-account access token; OS Login requires ``users/{url-encoded email}``.
    """
...
def _maybe_compute_image_delete_debug(token: str) -> None:
    """If COMPUTE_IMAGE_DELETE_DEBUG=true, stderr who the token belongs to (tokeninfo); never stdout."""
    if os.environ.get("COMPUTE_IMAGE_DELETE_DEBUG", "").strip().lower() != "true":
        return
    url = "https://oauth2.googleapis.com/tokeninfo?" + urllib.parse.urlencode(
        {"access_token": token},
    )
    ...
    *,
    ...
    for _ in range(max_iterations):
        ...
        if isinstance(op_url, str) and op_url:
            ...
            name = op.get("name")
            if not name:
                return False, "operation missing name in %s" % json.dumps(op)[:500]
...
def cmd_get_instance_template(project: str, region: str, template_name: str, token: str) -> int:
    """GET regional then global instanceTemplates/{name}; exit 0 if either is HTTP 200."""
    regional_url = (
        "https://compute.googleapis.com/compute/v1/projects/"
        + project
        + "/regions/"
    ...
    try:
        ...
        sys.stderr.write("get regional instance template request failed: %s\n" % e)
        return 1
    if st == 200:
        return 0
    if st != 404:
    ...
    base = "https://compute.googleapis.com/compute/v1/projects/" + project + "/global"
    global_url = base + "/instanceTemplates/" + template_name
    try:
        st_g, body_g = _http("GET", global_url, token, timeout=60)
    except OSError as e:
        sys.stderr.write("get global instance template request failed: %s\n" % e)
        return 1
    if st_g == 200:
        return 0
    if st_g == 404:
        sys.stderr.write(
            "instanceTemplates.get: %r not found in region %s or globally in project %s\n"
...
def cmd_delete_global_image(project: str, image_name: str, token: str) -> int:
    """Best-effort global image delete; single machine-readable stdout line (see module doc)."""
    ...
    ok, err = _wait_operation_done(op, token, project, max_iterations=120, sleep_s=2.0, get_timeout=180)
    if ok:
        print("removed")
        return 0
...
def cmd_delete_instance_template(project: str, template_name: str, token: str) -> int:
    """Strict-ish global instance template delete: exit 1 on real failure (see module doc)."""
    base = "https://compute.googleapis.com/compute/v1/projects/" + project + "/global"
    st, body = _http("DELETE", base + "/instanceTemplates/" + template_name, token, timeout=180)
    if st == 404:
        return 0
    if st != 200:
        sys.stderr.write(
...
def cmd_delete_regional_instance_template(project: str, region: str, template_name: str, token: str) -> int:
    """Regional instance template delete (KCC spec.region); 404 => success. Same polling as global."""
    base = (
        "https://compute.googleapis.com/compute/v1/projects/"
        + project
        + "/regions/"
    ...
    if ok:
        return 0
    if err == "operation timeout":
        sys.stderr.write("regional instance template delete operation timeout\n")
    else:
        sys.stderr.write(err + "\n")
    return 1


def _list_zone_disks(project: str, zone: str, token: str) -> tuple[list[dict], str | None]:
    """Paginated ``GET .../zones/{zone}/disks`` (Compute v1 ``disks.list``).

    Returns ``(disk_resource_dicts, error_message)``. On first non-200 or transport
    error, returns ``([], "<message>")``; on success, ``error_message`` is ``None``.
    """
    items: list[dict] = []
    page_token: str | None = ""
    while True:
...
def _disk_size_gb(disk: dict) -> int | None:
    """Parse Compute ``sizeGb`` string field; ``None`` if missing or invalid."""
    raw = disk.get("sizeGb")
    if raw is None:
        return None
    try:
...
def _parse_orphan_packer_size_gb(size_gb_str: str) -> tuple[int | None, str | None]:
    """Return ``(want_size_gb, None)`` or ``(None, None)`` for any-size keyword.

    Keywords ``any``, ``*``, ``all`` (case-insensitive) mean do not filter on size.
    On invalid integer (and not a keyword), returns ``(None, "<error>")``.
    """
...
def _orphan_packer_disk_names(items: list[dict], want_size_gb: int | None) -> list[str]:
    """Names of zonal disks likely left by the Packer googlecompute builder.

    Keeps disks whose name starts with ``packer-``, ``users`` is absent/null/``[]``
    (unattached; non-list ``users`` is skipped). When ``want_size_gb`` is set,
    ``sizeGb`` must equal it; when ``None``, any positive parsed size matches.
    Disks with missing or invalid ``sizeGb`` are skipped.
    """
    out: list[str] = []
    for d in items:
        name = d.get("name")
...
def _log_orphan_packer_disks_if_any(cmd: str, names: list[str]) -> None:
    """One stderr line when unattached packer-* disks match (otherwise silent)."""
    if not names:
        return
    sys.stderr.write("%s: %d orphan packer-* disk(s): %s\n" % (cmd, len(names), ", ".join(names)))


def cmd_list_orphan_packer_disks(project: str, zone: str, size_gb_str: str, token: str) -> int:
...
def cmd_delete_zonal_disk(project: str, zone: str, disk_name: str, token: str) -> int:
    ...
    try:
        ...
        sys.stderr.write("disks.delete request failed for %s: %s\n" % (disk_name, e))
        return 0
    if st == 404:
        return 0
    if st != 200:
        ...
        return 0
    op = json.loads(body) if body.strip() else {}
    ok, err = _wait_operation_done(op, token, project, max_iterations=180, get_timeout=300)
    if not ok:
        sys.stderr.write("disks.delete op for %s: %s\n" % (disk_name, err))
    return 0


def cmd_cleanup_orphan_packer_disks(project: str, zone: str, size_gb_str: str, token: str) -> int:
    """CLI: list+delete matching disks; always exit 0 (see module docstring)."""
    want, parse_err = _parse_orphan_packer_size_gb(size_gb_str)
    if parse_err:
...
def cmd_zone_instance(action: str, project: str, zone: str, instance_name: str, token: str) -> int:
    """Zonal stop/delete; always exit 0 like the historical shell wrappers."""
    base = "https://compute.googleapis.com/compute/v1/projects/" + project + "/zones/" + zone
    if action == "stop":
        st, body = _http(
            "POST",
...
def cmd_prune_os_login_ssh_keys(token: str) -> int:
    """Remove all SSH public keys from the token identity's OS Login profile."""
    token_email = _access_token_email(token)
    url = _os_login_login_profile_url(token)
    try:
        st, body = _http("GET", url, token, timeout=60)
    except OSError as e:
        sys.stderr.write("prune-os-login: loginProfile request failed: %s\n" % e)
        print("0")
        return 0
    if st != 200:
        sys.stderr.write(
    ...
    for map_key, entry in keys_obj.items():
        if not isinstance(entry, dict):
            continue
        name = entry.get("name")
        # getLoginProfile returns sshPublicKeys[].name as users/{numericId}/sshPublicKeys/{fp}.
        # DELETE with a workload-identity GSA token must use the same user segment as the
        # credential (service account email), not the numeric id, or OS Login returns 403.
        ...
        if not fingerprint:
            sys.stderr.write(
                "prune-os-login: sshPublicKey missing fingerprint, skip one entry\n",
            )
            continue
        ...
        if token_email:
            ...
            enc = "/".join(urllib.parse.quote(seg, safe="") for seg in name.split("/"))
            del_url = "https://oslogin.googleapis.com/v1/" + enc
        else:
            sys.stderr.write("prune-os-login: sshPublicKey missing name, skip one entry\n")
            continue
        try:
            st_d, body_d = _http("DELETE", del_url, token, timeout=60)
        except OSError as e:
            sys.stderr.write("prune-os-login: delete %s failed: %s\n" % (del_url, e))
            continue
        if st_d in (200, 204):
            removed += 1
            continue
        if st_d == 404:
            continue
        sys.stderr.write(
            "prune-os-login: delete %s HTTP %s: %s\n" % (del_url, st_d, body_d[:800]),
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "GCP REST helpers for cf_create_instance_template.sh — Compute Engine v1 "
            "and Cloud OS Login v1; metadata bearer token only; see module docstring."
        ),
        ...
        epilog=(
            ...
            "Optional: COMPUTE_IMAGE_DELETE_DEBUG=true (delete-global-image only)."
        ),
    )
    ...
    p_img = sub.add_parser(
        "delete-global-image",
        help="DELETE global/images/{name}; poll async Operation when API returns one.",
    )
    ...
    p_tmpl_get = sub.add_parser(
        ...
        help="GET regional then global instanceTemplates/{name}; exit 0 if either exists.",
    )
    ...
    p_tmpl_reg = sub.add_parser(
        ...
        help="DELETE regions/{region}/instanceTemplates/{name} and poll until DONE.",
    )
    ...
    p_zone = sub.add_parser(
        "zone-instance",
        help="POST instances.stop or DELETE instances (best-effort; exits 0 like legacy shell).",
    )
    ...
    p_list_orphan = sub.add_parser(
        ...
        help="List unattached packer-* disks (size_gb integer, or any/* /all for all sizes).",
    )
    ...
    p_del_disk = sub.add_parser(
        "delete-zonal-disk",
        help="DELETE zones/{zone}/disks/{name}; poll Operation (best-effort exit 0).",
    )
    ...
    p_cleanup_orphan = sub.add_parser(
        ...
        help="Delete unattached packer-* disks (size_gb or any/* /all; best-effort exit 0).",
    )
    ...
    sub.add_parser(
        ...
        help="DELETE sshPublicKeys on token identity loginProfile (tokeninfo email; best-effort).",
    )
    ...
    if args.command == "delete-instance-template":
        return cmd_delete_instance_template(args.project, args.template_name, token)
    ...
    if args.command == "list-orphan-packer-disks":
        return cmd_list_orphan_packer_disks(args.project, args.zone, args.size_gb, token)
    if args.command == "delete-zonal-disk":
        return cmd_delete_zonal_disk(args.project, args.zone, args.disk_name, token)
    if args.command == "cleanup-orphan-packer-disks":
        return cmd_cleanup_orphan_packer_disks(args.project, args.zone, args.size_gb, token)
    return 1
